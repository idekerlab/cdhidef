...
def _parse_arguments(desc, args):
    ...
    parser.add_argument('--p', default=75, type=int,
                        help='Consensus threshold. Threshold of '
                             ...
                             'choose gene as for each community')
    ...
    parser.add_argument('--convertresult', action='store_true',
                        help='If set, it is assumed the <input> passed in is the '
                             'path to the .nodes or .edges file generated by hidef '
                             'and this program will just parse those results instead '
                             'of running hidef')
    parser.add_argument('--includemapping', action='store_true',
                        help='If set, includes clusterMapping map to '
                             'JSON output that links how clusters are mapped to node ids')
...
def write_cluster_mapping(out_stream, cluster_mapping):
    if cluster_mapping is None:
        return
    out_stream.write(' "clusterMapping": {')
    is_first = True
...
def run_hidef(theargs, out_stream=sys.stdout,
    ...
    if theargs.convertresult is True:
        if not theargs.input.endswith('.nodes') and not theargs.input.endswith('.edges'):
            err_stream.write('--convertresult expects input to be .nodes or .edges output '
                             'file from hidef that was run outside this program')
            return 6
        rundir = os.path.dirname(theargs.input)
        prefix = os.path.basename(theargs.input)[0:-6]
        try:
            convert_hidef_output_to_cdaps(out_stream, rundir, prefix=prefix,
                                          include_mapping=theargs.includemapping)
